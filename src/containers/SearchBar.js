...
class SearchBar extends Component {
    constructor(props) {
        ...
        this.state = { term: ''}; //term is gonna be the name that we give for the search term.
        // means: the value of the input is mapped to this.state.term.
        ...
        this.onFormSubmit = this.onFormSubmit.bind(this); //same thing
    }
    ...
    onFormSubmit(event) {
        ...
        // input by click enter for example). And then we write how to fetch weather data here underneath.
        //The call of the fetchWeather Action Creator:
        ...
        //it will cause our component to re-render and make the input field to clear out.  
    }
    
    // after initializing the state, we can go ahead and update it over time here, using a
        ...
        return (
            <form onSubmit={this.onFormSubmit} className="input-group">
                <input 
                    ...
                    //that's where we turn it into a controlled component.
                    //first we assign the value to it, so its value comes from this.state.term
                    //and then whenever someone changes this value, run the function onInputChange
                    value={this.state.term}
                    onChange={this.onInputChange} //we define this callback function on our class SearchBar
                    //and then bind it to the SearchBar instance
                    //RULE: if we have a callback here, that makes reference to this, chances are we need to bind it! 
                    /> 
...
function mapDispatchToProps(dispatch) {
    ...
    //and returns an Action, bindActionCreators with dispatch makes sure that that Actions flows down into the 
